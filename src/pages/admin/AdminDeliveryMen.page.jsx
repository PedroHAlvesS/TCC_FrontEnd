...
export default function AdminDeliveryMenPage() {
	...
	useEffect(() => {
		...
		const deliveryManToEdit = deliveryMen.find(
			(item) => String(item.id) === String(editDeliveryManId),
		);
	...
	async function handleCreateDeliveryMan() {
		...
		try {
			...
			setDeliveryMen(
				Array.isArray(updatedDeliveryMen) ? updatedDeliveryMen : [],
			);
		} catch (err) {
	...
	async function handleUpdateDeliveryMan() {
		...
		const validation = validateProfile(
			{ email: newEmail, phone: newPhone, password: newPassword },
			...
			<div className="dashboard-main">
						...
						{profileOpen && (
							<div className="profile-dropdown">
								<button
									...
									onClick={() => navigate(ROUTES.ADMIN_EDIT_PROFILE)}
								>
									Editar Perfil
								</button>
								<button
									type="button"
									className="profile-dropdown-item"
									onClick={() => navigate(ROUTES.ADMIN_CREATE_PROFILE)}
								>
									Criar Novo Perfil
								</button>
								<button
											...
											<td>
												<div className="table-actions">
													<button
														...
														onClick={() =>
															navigate(
																buildRoute(ROUTES.ADMIN_DELIVERY_MEN_ORDERS, {
																	id: deliveryMan.id,
																}),
															)
														}
													...
													<button
														type="button"
														className="action-button"
														onClick={() => openEditDrawer(deliveryMan)}
													>
														✏️
													</button>
													<button
														...
														onClick={() => handleDeleteDeliveryMan(deliveryMan)}
													>
														🗑️
													</button>
												</div>
				...
				<aside className={`delivery-side-panel ${drawerOpen ? "open" : ""}`}>
					<div className="delivery-side-panel-header">
						<div>
							<h2>
								{editingDeliveryMan ? "Editar Entregador" : "Novo Entregador"}
							</h2>
							<p>
								{editingDeliveryMan
									? "Atualize apenas o e-mail, senha e telefone do entregador."
									: "Preencha os dados abaixo para cadastrar um novo entregador."}
							</p>
						</div>
						<button
							type="button"
