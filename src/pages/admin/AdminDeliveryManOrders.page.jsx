...
export default function AdminDeliveryManOrdersPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchAdminProfile(), fetchDeliveryMen(), fetchDeliveryManOrders(id)])
      .then(([adminData, deliveryMenData, orderData]) => {
        if (!mounted) return;
        setAdmin(adminData);
        const foundDeliveryMan = (deliveryMenData || []).find((item) => String(item.id) === String(id));
        if (!foundDeliveryMan) {
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((order) => {
      const matchesName = order.customerName.toLowerCase().includes(searchClient.toLowerCase());
