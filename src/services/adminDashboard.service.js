...
export async function fetchDeliveryMen() {
	...
	const res = await fetch(
		`${import.meta.env.VITE_BACKEND_URL}/api/delivery-men`,
		{
			headers: token
				? {
						Authorization: `Bearer ${token}`,
...
export async function updateDeliveryMan(deliveryManId, deliveryData) {
	const token = getToken();
	const res = await fetch(
		`${import.meta.env.VITE_BACKEND_URL}/api/delivery-men/${deliveryManId}`,
		{
			method: "PUT",
			headers: token
				? {
...
export async function fetchDeliveryManOrders(deliveryManId) {
	const token = getToken();
	const res = await fetch(
		`${import.meta.env.VITE_BACKEND_URL}/api/orders/delivery-man/${deliveryManId}`,
		{
			headers: token
				? {
						Authorization: `Bearer ${token}`,
...
export async function fetchCustomerOrders(customerId) {
	const token = getToken();
	const res = await fetch(
		`${import.meta.env.VITE_BACKEND_URL}/api/orders/customer/${customerId}`,
		{
			headers: token
				? {
						Authorization: `Bearer ${token}`,
...
export async function assignOrderToDeliveryMan(orderId, deliveryManId) {
	const token = getToken();
	const res = await fetch(
		`${import.meta.env.VITE_BACKEND_URL}/api/orders/${orderId}/assign`,
		{
			method: "PUT",
			headers: token
				? {
