...
export async function fetchDeliveryManOrders() {
	...
	const res = await fetch(
		`${import.meta.env.VITE_BACKEND_URL}/api/orders/delivery-man`,
		{
			headers: token
				? {
...
export async function fetchDeliveryManProfile() {
	const token = getToken();
	const res = await fetch(
		`${import.meta.env.VITE_BACKEND_URL}/api/delivery-men/email`,
		{
			headers: token
				? {
						Authorization: `Bearer ${token}`,
...
export async function updateDeliveryManOrderStatus(orderId, status) {
	const token = getToken();
	const res = await fetch(
		`${import.meta.env.VITE_BACKEND_URL}/api/orders/delivery-man/${orderId}/status`,
		{
			method: "PUT",
			headers: token
				? {
